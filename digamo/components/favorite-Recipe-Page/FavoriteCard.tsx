"use client";
import { FaHeart } from "react-icons/fa";


const FavoriteRecipePage = () => {
    return (
        <div className=" bg-gradient-to-tl from-[#F0B60E]/30 to-[#B8D4C8]/30 
        w-full h-full rounded-2xl p-5 flex flex-col justify-between
        shadow-[0_5px_5px_rgba(255,255,255,0.5)] hover:scale-105 transition-all duration-200 hover:cursor-pointer">
            <div className="flex items-end justify-end mb-2">
                <FaHeart className="text-[#F36B3F] text-lg" />
            </div>
            {/*Title*/}
            <h1 className="text-[#F7931E] text-2xl font-bold break-words mt-5 mr-5">Testingbgljsfbglsfbglsfbgkgjbsjsessdtesting t</h1>
            
            <div className="flex flex-row justify-between items-end mt-10">
                <h2 className="text-md font-extralight">Date: 1/3/25</h2>
                <button className="w-30 h-10 bg-gray-500 rounded-2xl bg-gradient-to-l from-[#EF4444] to-[#F36B3F]
                border border-white hover:cursor-pointer active:translate-y-1 transition-all
                ">View Recipe</button>
            </div>
        </div>
    );
}

export default FavoriteRecipePage;