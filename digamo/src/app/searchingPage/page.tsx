    ...
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="font-quattrocento text-lg animate-pulse">
          Digamo is creating recipes for: <span className="font-bold">{query}</span>...
        </p>
      </div>
    );
  }
          ...
          <div className="p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {recipe.matchedIngredients.map((ing, i) => (
                <span key={`m-${i}`} className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium">
                  ✓ {ing}
                </span>
              ))}
              {/* Missing */}
              {recipe.missingIngredients.map((ing, i) => (
                <span key={`mis-${i}`} className="px-3 py-1 bg-red-50 text-red-600 rounded-full text-sm">
                  + {ing}
                </span>
              ))}
            </div>
